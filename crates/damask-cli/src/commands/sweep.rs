...
pub fn run(reanchor: bool, format: Format) -> Result<()> {
    ...
    for e in &open_edges {
        for id in [e.from_id.as_deref(), e.to_id.as_deref()].into_iter().flatten() {
    ...
    for s in spans {
        let Some(&n) = open_by_span.get(&s.id) else { continue };
    ...
    if reanchor {
        ...
        for (row, _) in &drifted {
            if let Some(span) = helpers::reanchor_span(&project, row) {
                by_ns.entry(span.ns.clone()).or_default().push(Fact::Span(span));
                count += 1;
            }
        }
        ...
        match format {
            Format::Human => {
                println!("Re-anchored {count} drifted span(s) — they are fresh (\u{2705}) as of HEAD.");
                if !gone.is_empty() {
                    println!(
        ...
        Format::Human => {
            ...
            if !gone.is_empty() {
                ...
                for (s, n) in gone.iter().take(8) {
                    println!("    {}  ({} edge{})", s.path, n, if *n == 1 { "" } else { "s" });
                }
                if gone.len() > 8 {
